					...
					switch(answer3){
						...
						case 2 -> {
							...
							for(Professor z: allProfessors){
								existing.add(z.getName().toLowerCase());
							}
							...
							for(Professor p: loadedProfessors){
								if(!existing.contains(p.getName().toLowerCase())){
									allProfessors.add(p);
									ucf.addProfessor(p);
								}
							}
						...
						case 3 -> {
							...
							for(Course z: allCourses){
								existing.add(z.getCourseTitle().toLowerCase());
							}
							...
							for(Course p: loadedCourses){
								if(!existing.contains(p.getCourseTitle().toLowerCase())){
									allCourses.add(p);
									ucf.addCourse(p);
								}
							}
